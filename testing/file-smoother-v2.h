...
class FileSmootherV2 : public SmootherBase
{
    typedef FileSmootherV2 Me; // Required by SSLOT macro
...
public:
    FileSmootherV2(CUDT* parent) : SmootherBase(parent)
    {
    ...
    void updateBandwidth(int64_t maxbw, int64_t) ATR_OVERRIDE
    {
        ...
        {
            m_maxSR = maxbw;
            HLOGC(mglog.Debug, log << "FileSmootherV2: updated BW: " << m_maxSR);
        }
    }
...
private:
    ...
    void updateSndPeriod(ETransmissionEvent, EventVariant arg)
    {
        ...
        if (m_bSlowStart)
        {
            ...
            if (m_dCWndSize > m_dMaxCWndSize)
            {
                ...
                if (m_parent->deliveryRate() > 0)
                {
                    ...
                    LOGC(mglog.Debug, log << "FileSmootherV2: UPD (slowstart:ENDED) wndsize="
            ...
            {
                LOGC(mglog.Debug, log << "FileSmootherV2: UPD (slowstart:KEPT) wndsize="
                    << m_dCWndSize << "/" << m_dMaxCWndSize
        ...
        else
        {
            ...
            LOGC(mglog.Debug, log << "FileSmootherV2: UPD (speed mode) wndsize="
        ...
        LOGC(mglog.Debug, log << "FileSmootherV2: UPD (slowstart:"
        ...
        if (m_maxSR)
        {
            ...
            if (m_dPktSndPeriod < minSP)
            {
                ...
                LOGC(mglog.Debug, log << "FileSmootherV2: BW limited to " << m_maxSR
    ...
    void slowdownSndPeriod(ETransmissionEvent, EventVariant arg)
    {
        ...
        if (losslist_size == 0)
        {
            LOGC(mglog.Error, log << "IPE: FileSmootherV2: empty loss list!");
            return;
        }
        ...
        if (m_bSlowStart)
        {
            ...
            {
                m_dPktSndPeriod = 1000000.0 / m_parent->deliveryRate();
                LOGC(mglog.Debug, log << "FileSmootherV2: LOSS, SLOWSTART:OFF, sndperiod=" << m_dPktSndPeriod << "us AS mega/rate (rate="
                    << m_parent->deliveryRate() << ")");
            }
            else
            {
                m_dPktSndPeriod = m_dCWndSize / (m_parent->RTT() + m_iRCInterval);
                LOGC(mglog.Debug, log << "FileSmootherV2: LOSS, SLOWSTART:OFF, sndperiod=" << m_dPktSndPeriod << "us AS wndsize/(RTT+RCIV) (RTT="
                    << m_parent->RTT() << " RCIV=" << m_iRCInterval << ")");
            }
        ...
        // using FileSmootherV2, so relying on SRTO_TRANSTYPE rather than
    ...
    void speedupToWindowSize(ETransmissionEvent, EventVariant arg)
    {
        ...
        if (m_bSlowStart)
        {
            ...
            {
                m_dPktSndPeriod = 1000000.0 / m_parent->deliveryRate();
                LOGC(mglog.Debug, log << "FileSmootherV2: CHKTIMER, SLOWSTART:OFF, sndperiod=" << m_dPktSndPeriod << "us AS mega/rate (rate="
                    << m_parent->deliveryRate() << ")");
            }
            else
            {
                m_dPktSndPeriod = m_dCWndSize / (m_parent->RTT() + m_iRCInterval);
                LOGC(mglog.Debug, log << "FileSmootherV2: CHKTIMER, SLOWSTART:OFF, sndperiod=" << m_dPktSndPeriod << "us AS wndsize/(RTT+RCIV) (wndsize="
