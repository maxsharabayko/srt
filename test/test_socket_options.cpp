...
TEST_F(TestSocketOptions, Latency)
{
    ...
    ASSERT_EQ(srt_setsockopt(m_caller_sock, 0, SRTO_LATENCY, &latency, sizeof latency), SRT_SUCCESS);
    ...
    int opt_val = 0;
    int opt_len = 0;
    ASSERT_EQ(srt_getsockopt(accepted_sock, 0, SRTO_RCVLATENCY, &opt_val, &opt_len), SRT_SUCCESS);
    cerr << "Accepted socket:" << endl;
    cerr << "SRTO_RCVLATENCY = " << opt_val << endl;
    ASSERT_EQ(srt_getsockopt(accepted_sock, 0, SRTO_PEERLATENCY, &opt_val, &opt_len), SRT_SUCCESS);
    cerr << "SRTO_PEERLATENCY = " << opt_val << endl;
    ASSERT_EQ(srt_getsockopt(accepted_sock, 0, SRTO_LATENCY, &opt_val, &opt_len), SRT_SUCCESS);
    cerr << "SRTO_LATENCY = " << opt_val << endl;

    ASSERT_EQ(srt_getsockopt(m_caller_sock, 0, SRTO_RCVLATENCY, &opt_val, &opt_len), SRT_SUCCESS);
    cerr << "Caller socket:" << endl;
    cerr << "SRTO_RCVLATENCY = " << opt_val << endl;
    ASSERT_EQ(srt_getsockopt(m_caller_sock, 0, SRTO_PEERLATENCY, &opt_val, &opt_len), SRT_SUCCESS);
    cerr << "SRTO_PEERLATENCY = " << opt_val << endl;
    ASSERT_EQ(srt_getsockopt(m_caller_sock, 0, SRTO_LATENCY, &opt_val, &opt_len), SRT_SUCCESS);
    cerr << "SRTO_LATENCY = " << opt_val << endl;
